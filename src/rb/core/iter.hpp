...
namespace impl {
	...
	template <class T>
	using ValueTypeDetector = typename std::iterator_traits<T>::value_type;

	template <class T>
	using IteratorCategoryDetector = typename std::iterator_traits<T>::iterator_category;
...
template <class T>
using IsInputIterator = IsDetectedConvertible<std::input_iterator_tag, impl::IteratorCategoryDetector, T>;
...
template <class T>
using IterReference = decltype(*RB_DECLVAL(T&));
